...
trimestre_a_numeric <- function(x) {
  recode(tolower(x), "i" = 1, "ii" = 2, "iii" = 3, "iv" = 4)
}

trimestre_a_mes <- function(x) {
  recode(as.character(x), "1" = 3 * 1, "2" = 3 * 2, "3" = 3 * 3, "4" = 3 * 4)
}
...
limpiar_tabla_bc <- function(
  ...
  if (frecuencia == "mensual") {
    ...
    dato_1b <- dato_1a |>
      mutate(
        trimestre = str_extract(fecha, "^\\w+(?=_)"),
  ...
  tryCatch(
    {
      ...
      dato_1 <- scrapear_tabla_bc(
        "https://si3.bcentral.cl/Siete/ES/Siete/Cuadro/CAP_CCNN/MN_CCNN76/CCNN2018_P0_V2/637801082315858005",
        convertir = FALSE
      )
...
obtener_desocupados <- function() {
  message(
    "obtienendo desocupados (INE, promedios móviles trimestrales, miles de personas) desde web del Banco Central..."
  )
  ...
  dato_1 <- scrapear_tabla_bc(
    "https://si3.bcentral.cl/Siete/ES/Siete/Cuadro/CAP_EMP_REM_DEM/MN_EMP_REM_DEM13/ED_FTM2/a6"
  )
...
obtener_remuneraciones <- function() {
  message(
    "obtienendo índice de remuneraciones INE (base 2023=100) desde web del Banco Central..."
  )
  ...
  dato_1 <- scrapear_tabla_bc(
    "https://si3.bcentral.cl/Siete/ES/Siete/Cuadro/CAP_EMP_REM_DEM/MN_EMP_REM_DEM13/ED_IND_REM_M_2023/638532110891789445"
  )
...
obtener_canasta <- function() {
  message(
    "obteniendo canasta básica de alimentos desde GitHub (bastianolea/canasta_basica_chile)"
  )
...
obtener_sueldo_minimo <- function() {
  ...
  sueldo_minimo_2 <- sueldo_minimo_1 |>
    mutate(
      monto_bruto = str_remove(monto_bruto, "pesos") |> str_trim(),
    ...
    mutate(
      salario_real = str_replace(salario_real, ",", "\\."),
  ...
  sueldo_minimo_3 <- sueldo_minimo_2 |>
    ...
    filter(año >= 2000, monto_bruto >= 100000, !is.na(monto_bruto))
...
descargar_inversion_extranjera <- function() {
  message(
    "descargando inversión extranjera (Excel) desde web del Banco Central..."
  )
...
obtener_inversion_extranjera <- function(descargar = TRUE) {
  ...
  invext <- read_excel(
    "fuentes/bc_inv_extranjera/Cuenta_Financiera_categoria.xlsx",
  ...
  invext_2 <- invext_0 |>
    ...
    mutate(
      ubicacion = case_when(
        str_detect(col_1, "(I|i)nversión.*(d|D)irecta") ~ TRUE,
  ...
  # limpiar datos
  invext_6 <- invext_5 |>
    mutate(mes = str_to_lower(mes), mes = str_remove(mes, "\\.")) |>
...
cargar_precio_cobre_anterior <- function(año) {
  ...
  indicador_cobre_0 <- read_excel(
    paste0("fuentes/bc_precio_cobre/Indicador_", año, ".xls"),
    .name_repair = "unique_quiet"
  )
  ...
  indicador_cobre_2 <- indicador_cobre_1 |>
    filter(!is.na(valor), !is.na(año), !is.na(mes), !is.na(dia))
...
obtener_precio_cobre <- function() {
  ...
  return(tabla_cobre_5 |> select(serie, fecha, año, mes, valor))
}
