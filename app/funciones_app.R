...
cargar_datos_web <- function(
  ...
  if (descargar == TRUE) {
    url = paste0(
      "https://github.com/bastianolea/economia_chile/raw/main/app/datos/",
      archivo,
      ".csv"
    )
...
mes_a_trimestre <- function(x) {
  recode(as.character(x), "3" = "I", "6" = "II", "9" = "III", "12" = "IV")
}
...
porcentaje_flechita <- function(dato, juicio = "bueno") {
  div(
    ...
    div(
      style = "display: inline-block; vertical-align:middle; margin-left: -3px; margin-right: 0px; margin-bottom: 2px;",
      flechita(dato, juicio = juicio)
    ),
    div(
      style = "display: inline-block; vertical-align:middle; font-size: 200%; text-align: center;",
...
formateador_cifra <- function(
  ...
  if (unidad == "miles de millones") {
    ...
    paste(
      paste0("$", format(round(dato, 2), big.mark = ".", decimal.mark = ",")),
      "(dólares por libra)"
    )
  } else {
    # paste(paste0("$", miles(dato)), "(miles de millones)")
...
dato_ui <- function(
  ...
  output <- div(
    ...
    div(
      ...
      # cuadro izq
      div(
        style = estilo_texto_izquierdo,
        strong("Valor actual"),
        p(
          style = estilo_cifra_izquierda,
...
tendencia_ui <- function(datos, fecha_corte, input, subir = "bueno") {
  ...
  tendencia_cifra <- scales::percent(
    variacion - 1,
    big.mark = ".",
    decimal.mark = ",",
    accuracy = 0.1
...
tendencia_texto <- function(datos, fecha_corte, input) {
  ...
  if (dato == "pib") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que la economía del país está creciendo en la producción, inversión, y prestación de servicios",
      tendencia ==
        "disminuyó" ~ "Significa que la economía está decreciendo, debido a una baja en al producción, la inversión, o la prestación de servicios",
      tendencia ==
        "se mantuvo" ~ "Esto significa que la producción, inversión y prestación de servicios se ha mantenido"
    )
  } else if (dato == "imacec") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que la actividad de los distintos sectores de la economía del país está en aumento",
      tendencia ==
        "disminuyó" ~ "Significa que la actividad económica del país está bajando",
      tendencia ==
        "se mantuvo" ~ "Significa que la actividad económica del país se ha mantenido en los mismos niveles"
    )
  } else if (dato == "ipc") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Indica que los precios de los bienes y servicios más comunes han subido",
      tendencia ==
        "disminuyó" ~ "Indica que los precios de los bienes y servicios más comunes están bajando",
      tendencia ==
        "se mantuvo" ~ "Esto indica que los precios de los bienes y servicios más comunes se han mantenido"
    )
  } else if (dato == "inversion_extranjera") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que las empresas extranjeras están creando nuevas empresas en el país, y aumentando su inversión en empresas chilenas",
      tendencia ==
        "disminuyó" ~ "Significa que empresas extranjeras están invirtiendo menos en empresas o creación de empresas en el país",
      tendencia ==
        "se mantuvo" ~ "Significa que el nivel de inversión de las empresas extranjeras en crear nuevas empresas o controlar empresas chilenas se ha mantenido"
    )
  } else if (dato == "uf") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Refleja que el precio de bienes y servicios ha subido, por lo que la UF se alinea a estos aumentos",
      tendencia ==
        "disminuyó" ~ "Refleja que el precio de bienes y servicios ha bajado, por lo que la UF se alinea a esta disminución",
      tendencia ==
        "se mantuvo" ~ "Refleja que el precio de bienes y servicios se ha mantenido estable"
    )
  } else if (dato == "ipsa") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que mejoró el desempeño de las acciones de las mayores compañías del país, reflejando crecimiento económico",
      tendencia ==
        "disminuyó" ~ "Significa que el desempeño de las acciones de las mayores compañías del país está empeorando, indicando recesión económica",
      tendencia ==
        "se mantuvo" ~ "Significa que el desempeño de las acciones de las mayores compañías del país se ha mantenido estable"
    )
  } else if (dato == "desempleo") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que ha aumentado la cantidad de personas sin trabajo",
      tendencia ==
        "disminuyó" ~ "Significa que hay más puestos de trabajo en Chile, y por lo tanto está disminuyendo la cantidad de personas sin trabajo",
      tendencia ==
        "se mantuvo" ~ "Significa que la cantidad de chilenos/as sin trabajo se ha mantenido"
    )
  } else if (dato == "remuneraciones") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Significa que los ingresos de los chilenos están aumentando, a pesar de la inflación",
      tendencia ==
        "disminuyó" ~ "Significa que los ingresos de los chilenos están bajando, producto de la inflación",
      tendencia ==
        "se mantuvo" ~ "Significa que los ingresos de los chilenos han aumentado lo suficiente como para anular el efecto negativo de la inflación"
    )
  } else if (dato == "precio_cobre") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Indica que el cobre está subiendo de precio a nivel mundial, beneficiando el presupuesto del Estado y la planificación económica a futuro",
      tendencia ==
        "disminuyó" ~ "Indica que el precio del cobre está bajando en el mercado mundial, lo que afectará el presupuesto del Estado a futuro",
      tendencia ==
        "se mantuvo" ~ "Indica que el precio del cobre a nivel mundial se ha mantenido, lo que significa estabilidad para los presupuestos nacionales"
    )
  } else if (dato == "prod_industrial") {
    texto <- case_when(
      tendencia ==
        "aumentó" ~ "Indica que el volumen de la producción nacional en actividades como minería, manufactura y energías está aumentando",
      tendencia ==
        "disminuyó" ~ "Indica que el volumen de la producción nacional en actividades como minería, manufactura y energías se ha mantenido constante",
      tendencia ==
        "se mantuvo" ~ "Indica que el volumen de la producción nacional en actividades como minería, manufactura y energías está disminuyendo"
    )
  }
...
panel <- function(width, ...) {
  column(width, class = "outer-panel", div(class = "panel", ...))
}
...
panel_vacio <- function(width, ...) {
  column(width, class = "outer-panel", div(class = "panel-vacio", ...))
}
...
panel_tendencia <- function(
  ...
  panel_texto(
    ...
    div(
      style = paste(
        "display: inline-block; margin-right: 4px; color:",
        color_detalle,
        ";"
      ),
      p("⏺︎︎")
    ),
    div(style = "display: inline-block;", p(texto)),
...
grafico_variacion <- function(
  ...
  dato_2 <- dato$variacion |>
    ...
    mutate(
      direccion = ifelse(valor > 0, "Aumento", "Disminución"),
  ...
  color_gradiente = if_else(
    n_aumentos > n_disminuciones,
    color_subir,
    color_bajar
  )
  colores_gradiente = c(color_gradiente, color_secundario)
  colores_gradiente_pos = if_else(
    gradiente_posicion == "arriba",
    list(colores_gradiente),
    list(rev(colores_gradiente))
  )
  posicion_y_gradiente = if_else(
    gradiente_posicion == "arriba",
    alto_max * 0.8,
    -alto_max * 0.8
  )
  ...
  plot <- dato_2 |>
    ...
    geom_ribbon(
      aes(
        x = c(
          fecha[1] + espaciado_meses,
          fecha[2:11],
          fecha[12] - espaciado_meses
        ),
        ymin = 0,
        ymax = posicion_y_gradiente
      ), #max(valor)),
    ...
    geom_text(
      data = ~ filter(.x, direccion == "Aumento"),
      aes(
        y = valor + (mean(valor) * espaciado_vertical_texto),
        label = scales::percent(
          valor,
          big.mark = ".",
          decimal.mark = ",",
          accuracy = decimales_texto
        )
      ),
      color = color_fondo,
      vjust = 0,
    ...
    geom_text(
      data = ~ filter(.x, direccion == "Disminución"),
      aes(
        y = valor + (mean(valor) * espaciado_vertical_texto),
        label = scales::percent(
          valor,
    ...
    scale_x_date(
      date_breaks = "months",
      labels = ~ numeric_a_mes(month(.x)),
      expand = expansion(0)
    ) +
    scale_y_continuous(
      limits = c(-alto_max, alto_max),
    ...
    theme_void() +
    theme(
      text = element_text(color = color_fondo),
      axis.text.x = element_text(),
      axis.text.y = element_text(margin = margin(r = 4)),
      panel.grid.major.y = element_line(
        linetype = "dashed",
        color = color_secundario_detalle
      ),
  ...
  if (escala == "trimestre") {
    plot <- plot +
      scale_x_date(
        breaks = dato_2$fecha,
        labels = ~ paste(year(.x), mes_a_trimestre(month(.x))),
        expand = expansion(0)
      ) +
...
grafico_historico <- function(dato, escala = "mensual") {
  ...
  disminucion = filter(datos_2, variacion_ultimo == min(variacion_ultimo)) |>
    slice(1)

  mayor_disminucion = filter(datos_2, variacion == min(variacion, na.rm = T)) |>
    slice(1)
  mayor_aumento = filter(datos_2, variacion == max(variacion, na.rm = T)) |>
    slice(1)
  ...
  datos_2 |>
    ...
    annotate(
      "point",
      x = mayor_aumento$fecha,
      y = mayor_aumento$valor,
      size = 9,
      alpha = .3,
    ...
    annotate(
      "point",
      x = mayor_disminucion$fecha,
      y = mayor_disminucion$valor,
      size = 9,
      alpha = .3,
    ...
    theme(
      plot.margin = margin(l = 15, r = 15, t = 4, b = 4),
      legend.position = "none"
    ) +
    theme(
      plot.background = element_rect(
