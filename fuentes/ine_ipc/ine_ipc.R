...
ipc <- readr::read_delim(
  "ine_ipc/ipc base anual 2018=100/Base anonimizada IPC 2019.csv",
  delim = '\\'
)
...
ipc_filt_2 <- ipc_filt |>
  ...
  mutate(
    ...
    mes_t = str_extract(
      fecha_t,
      "(?<=(pm_16_|pm_|Cantidad_|Unidad_))(.*?)(?=\\d{4})"
    ),
    mes = recode(
      tolower(mes_t),
      "enero" = "1",
